...
describe('Products', () => {
  ...
  it('Search Products and Verify Cart After Login', () => {
    ...
    cy.get(productsPages.cartModal).should('not.have.class', 'in')
  ...
  it('Add Multiple Products to Cart', () => {
    ...
    cy.contains(productsMessages.allProductsPageTitle).should('be.visible')
    cy.get(productsPages.productsList).should('be.visible')
    cy.get(productsPages.productItem).should('have.length.greaterThan', 2)
    cy.addProductToCart(0)
    ...
    cy.url().should('include', '/view_cart')
    cy.get(productsPages.cartTable).should('be.visible')
    cy.get(productsPages.cartProduct).should('have.length', 3)
  })

  it('Change Quantity on Product Details Page', () => {
    cy.get(globalPages.products).eq(0).click()
    cy.url().should('eq', `${Cypress.config().baseUrl}products`)
    cy.contains(productsMessages.allProductsPageTitle).should('be.visible')
    cy.get(productsPages.productItem).first().within(() => {
    ...
    cy.url().should('include', '/product_details/')
    cy.get(productsPages.productName).should('be.visible')
    cy.get(productsPages.quantityInput).clear().type('5')
    cy.contains(productsConsts.addToCart).click()
    cy.get(productsPages.continueShoppingButton).should('be.visible').click()
    cy.get(globalPages.cart).eq(0).click()
    cy.url().should('include', '/view_cart')
    cy.get(productsPages.cartTable).should('be.visible')
    cy.get(productsPages.cartProduct).should('have.length', 1)
  })
})
